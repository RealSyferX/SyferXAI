...
/**
 * Detect whether an error indicates the account has insufficient balance/credits.
 * Matches provider shapes like:
 *   {"code":"INSUFFICIENT_BALANCE","message":"Insufficient account balance"}
 *   {"error":{"message":"insufficient balance","type":"billing_error"}}
 * @param {number} status - HTTP status code (402/403 typical, not required)
 * @param {string|object} errorText - error body or message
 * @returns {boolean}
 */
export function isAutoRemovableConnectionError(status, errorText, customPatterns = []) {
  ...
  const unsupportedMaxOutputTokens =
    Number(status) === 400 &&
    text.includes("unsupported parameter") &&
    text.includes("max_output_tokens");
  const customMatch = Array.isArray(customPatterns) && customPatterns.some((pattern) => {
    const normalized = String(pattern || "").trim().toLowerCase();
    return normalized.length > 0 && text.includes(normalized);
  });
  return insufficientBalance || unsupportedMaxOutputTokens || customMatch;
}
