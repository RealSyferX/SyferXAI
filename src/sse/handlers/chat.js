...
import {
  ...
  isAutoRemovableConnectionError,
...
import { updateProviderConnection, getProviderConnections, deleteProviderConnection } from "@/lib/localDb";
...
async function handleSingleModelChat(body, modelStr, clientRawRequest = null, request = null, apiKey = null, apiKeyInfo = null, options = null) {
  ...
  while (true) {
    ...
    // Auto-remove unusable connections when enabled for this provider. Built-in
    // rules cover insufficient balance and max_output_tokens; users can add
    // case-insensitive substring patterns from the provider settings modal.
    let autoRemoveOverride = null;
    try {
      const s = await getSettings();
      autoRemoveOverride = (s?.providerStrategies || {})[provider] || null;
    } catch { /* settings unavailable → treat as disabled */ }

    if (
      autoRemoveOverride?.autoRemoveInsufficientBalance &&
      isAutoRemovableConnectionError(result.status, errorText, autoRemoveOverride.autoRemoveErrorPatterns)
    ) {
      try {
        await deleteProviderConnection(credentials.connectionId);
        log.warn("AUTH", `Auto-removed unusable connection ${credentials.connectionName || credentials.connectionId} from ${provider}: ${errorText}`);
      } catch (e) {
        log.error("AUTH", `Failed to auto-remove unusable connection: ${e.message}`);
      }
      // Exclude and continue — the deleted account won't be re-selected.
      excludeConnectionIds.add(credentials.connectionId);
      lastError = errorText;
