...
export default function ProviderDetailPage() {
  ...
  // sibling fields. Pass { fallbackStrategy, stickyRoundRobinLimit, autoRemoveInsufficientBalance }.
  const saveProviderOverride = async (patch) => {
    try {
      ...
      // Drop empty/false values so the override object stays clean.
      ...
      console.log("Error saving provider override:", error);
    }
  };
  ...
  const saveProviderStrategy = (strategy, stickyLimit) => {
    ...
    if (strategy === "round-robin" && stickyLimit !== "") {
      patch.stickyRoundRobinLimit = Number(stickyLimit) || 3;
    }
  ...
  const handleAutoRemoveBalanceToggle = (enabled) => {
    ...
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
        ...
        <Card>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
              ...
              {/* Auto Remove Insufficient Balance toggle */}
              <div className="flex flex-wrap items-center gap-2" title="Delete connections that return INSUFFICIENT_BALANCE or reject max_output_tokens, then let the remaining connections handle traffic.">
                <span className="text-xs text-text-muted font-medium">Auto Remove Insufficient Balance</span>
                <Toggle
                  checked={autoRemoveBalance}
                  onChange={handleAutoRemoveBalanceToggle}
                />
                <button
                  type="button"
                  onClick={() => setShowAutoRemoveSettings(true)}
                  className="inline-flex size-7 items-center justify-center rounded-md text-text-muted transition-colors hover:bg-surface-2 hover:text-primary"
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div>
            <label htmlFor="auto-remove-patterns" className="mb-1.5 block text-sm font-medium text-text-main">Custom error filters</label>
            <textarea
              id="auto-remove-patterns"
