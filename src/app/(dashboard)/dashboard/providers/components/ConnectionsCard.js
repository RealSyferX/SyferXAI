...
export default function ConnectionsCard({ providerId, isOAuth }) {
  ...
  const [autoRemoveBalance, setAutoRemoveBalance] = useState(false);
  const [showAutoRemoveSettings, setShowAutoRemoveSettings] = useState(false);
  const [autoRemovePatternsText, setAutoRemovePatternsText] = useState("");
  ...
  const fetch_ = useCallback(async () => {
    try {
      ...
      setAutoRemoveBalance(Boolean(override.autoRemoveInsufficientBalance));
      setAutoRemovePatternsText(Array.isArray(override.autoRemoveErrorPatterns) ? override.autoRemoveErrorPatterns.join("\n") : "");
  ...
  // Merge a partial override into providerStrategies[providerId] without clobbering
  // other fields. Pass { fallbackStrategy, stickyRoundRobinLimit, autoRemoveInsufficientBalance }.
  const saveProviderOverride = async (patch) => {
    try {
      const res = await fetch("/api/settings", { cache: "no-store" });
      const data = res.ok ? await res.json() : {};
      const current = data.providerStrategies || {};
      const merged = { ...(current[providerId] || {}), ...patch };

      // Drop empty/false/null values so the override object stays clean.
      if (!merged.fallbackStrategy) {
        delete merged.fallbackStrategy;
        delete merged.stickyRoundRobinLimit;
      }
      if (!merged.autoRemoveInsufficientBalance) delete merged.autoRemoveInsufficientBalance;
      if (!Array.isArray(merged.autoRemoveErrorPatterns) || merged.autoRemoveErrorPatterns.length === 0) {
        delete merged.autoRemoveErrorPatterns;
      }

      const updated = { ...current };
      if (Object.keys(merged).length === 0) delete updated[providerId];
      else updated[providerId] = merged;
    ...
    } catch (e) { console.log("saveProviderOverride error:", e); }
  };

  const saveStrategy = (strategy, stickyLimit) => {
    const patch = { fallbackStrategy: strategy || null };
    if (strategy === "round-robin" && stickyLimit !== "") patch.stickyRoundRobinLimit = Number(stickyLimit) || 3;
    return saveProviderOverride(patch);
  };

  const handleSaveAutoRemovePatterns = async () => {
    const seen = new Set();
    const patterns = autoRemovePatternsText
      .split(/\r?\n/)
    ...
    setAutoRemovePatternsText(patterns.join("\n"));
    await saveProviderOverride({ autoRemoveErrorPatterns: patterns });
    setShowAutoRemoveSettings(false);
  };
    ...
    <>
      <Card>
        <div className="flex flex-col gap-3 mb-4">
          ...
          {/* Auto Remove Insufficient Balance */}
          <div className="flex items-start gap-2.5 rounded-lg border border-border bg-black/[0.015] dark:bg-white/[0.015] px-3 py-2.5">
            <Toggle
              size="sm"
              checked={autoRemoveBalance}
              onChange={(enabled) => {
                setAutoRemoveBalance(enabled);
                saveProviderOverride({ autoRemoveInsufficientBalance: enabled });
              }}
            />
            <div className="flex flex-col">
              <span className="text-xs font-medium text-text-main">Auto Remove Insufficient Balance</span>
              <span className="text-[11px] text-text-muted leading-snug">
                When a connection returns <code className="font-mono">INSUFFICIENT_BALANCE</code> / insufficient balance, delete it automatically and let the rest handle traffic.
              </span>
            </div>
            <button
              type="button"
              onClick={() => setShowAutoRemoveSettings(true)}
              className="ml-auto inline-flex size-7 shrink-0 items-center justify-center rounded-md text-text-muted transition-colors hover:bg-surface-2 hover:text-primary"
              title="Configure custom auto-remove error patterns"
              aria-label="Configure custom auto-remove error patterns"
            >
      ...
      <Modal
        isOpen={showAutoRemoveSettings}
        title="Auto Remove Error Filters"
        onClose={() => setShowAutoRemoveSettings(false)}
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg border border-border bg-surface-2/50 p-3 text-xs leading-relaxed text-text-muted">
            Enter one case-insensitive error substring per line. A connection is deleted when its upstream error contains any line below. Built-in balance and <code className="font-mono text-text-main">max_output_tokens</code> rules always remain active while the toggle is on.
          </div>
          <div>
            <label htmlFor="connections-auto-remove-patterns" className="mb-1.5 block text-sm font-medium text-text-main">Custom error filters</label>
            <textarea
              id="connections-auto-remove-patterns"
              value={autoRemovePatternsText}
              onChange={(event) => setAutoRemovePatternsText(event.target.value)}
              rows={8}
              placeholder={"account suspended\ninvalid api key\nmodel access denied\ncredit exhausted"}
              className="w-full resize-y rounded-lg border border-border bg-bg px-3 py-2 font-mono text-sm text-text-main outline-none transition-colors placeholder:text-text-subtle focus:border-primary focus:ring-2 focus:ring-primary/15"
            />
            <p className="mt-1.5 text-xs text-text-muted">Use specific phrases to avoid deleting a connection for unrelated errors.</p>
          </div>
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-between">
            <Button variant="ghost" onClick={() => setAutoRemovePatternsText("")}>Clear</Button>
            <div className="flex gap-2">
              <Button variant="secondary" onClick={() => setShowAutoRemoveSettings(false)}>Cancel</Button>
              <Button variant="primary" onClick={handleSaveAutoRemovePatterns}>Save Filters</Button>
            </div>
          </div>
        </div>
      </Modal>
