...
import { checkFallbackError, isAutoRemovableConnectionError } from "../../open-sse/services/accountFallback.js";
...
describe("auto-removable connection errors", () => {
  it("matches an INSUFFICIENT_BALANCE code", () => {
    const body = '{"code":"INSUFFICIENT_BALANCE","message":"Insufficient account balance"}';
    expect(isAutoRemovableConnectionError(403, body)).toBe(true);
  });

  it("matches a billing_error insufficient balance message", () => {
    const body = '{"error":{"message":"insufficient balance","type":"billing_error"}}';
    expect(isAutoRemovableConnectionError(403, body)).toBe(true);
  });

  it("matches unsupported max_output_tokens on HTTP 400", () => {
    const body = '{"error":{"message":"Unsupported parameter: max_output_tokens","type":"invalid_request_error"}}';
    expect(isAutoRemovableConnectionError(400, body)).toBe(true);
  });

  it("does not match an unrelated unsupported parameter", () => {
    const body = '{"error":{"message":"Unsupported parameter: temperature","type":"invalid_request_error"}}';
    expect(isAutoRemovableConnectionError(400, body)).toBe(false);
  });

  it("matches a custom case-insensitive substring", () => {
    const body = '{"error":{"message":"This Account Has Been Suspended","type":"provider_error"}}';
    expect(isAutoRemovableConnectionError(403, body, ["account has been suspended"])).toBe(true);
  });
});
